...
async function ExportStruct(
  ...
  } --skip-comments > ${tempDir}/${dbInfo.name}_struct.sql`;
  ...
  handleExportStructSql(
    `${tempDir}/${dbInfo.name}_struct.sql`,
    exportConf.isForceUpdateTable,
    createDbSql,
  );

  // 返回导出的路径
  return `${tempDir}/${dbInfo.name}_struct.sql`;
}
...
async function ExportData(
  ...
  // 导出结构语句
  const exportComandData = `mysqldump --host=${dbConf.host}  -u${dbConf.user} -p${dbConf.password}  -P${dbConf.port}  --databases ${dbInfo.name}  --tables ${tableListStr} --no-create-info > ${tempDir}/${dbInfo.name}_data.sql`;
  //导出源库表结构 和 <数据>
  await execAsync(exportComandData);

  // 返回导出的路径
  return `${tempDir}/${dbInfo.name}_data.sql`;
}
